...
- `lportman link --force`: 別のフォルダを指しているジャンクションを張り直す (版上げで exe の場所が変わったとき)
- 修正: `reserve` が、予約先のプロジェクト自身が設定に書いたポートを「使用不可」として拒否していた
- ソースから動かすときに AI エージェントへ渡すコマンドを `uv run --no-sync` にした
  (画面を開いたまま依存が変わっていても、同期で失敗しない)
- 修正: 初回起動で調べるフォルダが未設定のままスキャンが走ると、あとでフォルダを選んだときに
  全部のポートが「新着」になっていた
- 解消案の一括予約で、同じプロジェクトの同じポート (compose と .env の両方にある場合など) を 1 回だけ登録する
- マニュアルの画面画像を、架空のプロジェクトを並べたデモ環境で撮り直して同梱した
- CI: main への push と pull request でテストを流す
