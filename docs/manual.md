...
### Claude に開発時のポートを管理させる

Claude Code などの AI エージェントは、新しいプロジェクトを既定の 5173 や 3000 のまま作りがちです。
次の手順で、ポートを決める前に LPortMan を使うよう指示できます。

1. [設定](#設定) で調べるフォルダを登録し、一度スキャンしておく
2. CLI で `lportman claude-md --write ~/.claude/CLAUDE.md` を実行する
   - `~/.claude/CLAUDE.md` は Claude Code が全プロジェクトで読む指示ファイル。1 つのプロジェクトだけに
     効かせたいなら、そのプロジェクトの `CLAUDE.md` を指定する
   - コマンドのパス (exe / uv tool / ソース) は自動で埋まる。`%USERPROFILE%\.lportman` も無ければ作られる
   - 2 回目からは LPortMan の節だけを差し替える。LPortMan を移動・入れ直したら再実行する
   - 書き込む前に中身を見たいときは `--write` を付けずに実行する
3. 新しい Claude のセッションから有効になる

書き込まれる指示で、Claude は場面ごとに次のように動きます。

| 場面 | Claude がすること |
|---|---|
| 新しいプロジェクト・開発サーバ・エミュレータを作る | `suggest` で候補を取り、設定ファイルに明記し、`reserve` で台帳に登録する |
| 作業を始める | `project <フォルダ>` で、そのプロジェクトのポート・衝突・台帳の反映状況を見る |
| 起動が「ポートが使用中」で失敗した | `check <port>` で持ち主を調べて伝える (プロセスは勝手に終了しない) |
| 設定ファイルのポートを書き換えた | `project <フォルダ>` で、予約が「反映済み」になったかを確かめる |
| 既存のポートを変えたい | 勝手に変えず、`plan` の解消案を示して相談する |

**効いているかの確かめ方**: 新しいセッションで「Vite のプロジェクトを作って」などと頼み、
Claude が `lportman suggest` を実行して、決めたポートを台帳に登録するかを見ます。
あとで [台帳](#台帳) タブに、そのプロジェクトの予約が「反映済み」で出ていれば成功です。
...
| `project [フォルダ] [--json]` | そのプロジェクトのポート・衝突・台帳の反映状況 (既定: 今のフォルダ) |
| `claude-md [--write ファイル]` | AI エージェント向けの CLAUDE.md の節を出力 / 書き込み |
...
| 参照先が別のフォルダを指している | 版上げで場所が変わった。`lportman link --force` で張り直し、使っているなら `lportman claude-md --write ~/.claude/CLAUDE.md` もやり直す |
| 新しい版にしたら台帳が空になった | exe 版は exe の隣の `data\` に保存する。前の版のフォルダから `data\` を移す (次からは同じフォルダに上書きで展開する) |
